"""
Comprehensive test: Compare SC List (SCL) and AI-Fast-SCL decoding across wider SNR range.
"""
import time
import numpy as np
...
def run_compare_comprehensive(N=128, K=64, messages=100, L=4):
    """Run comprehensive comparison test with wider SNR range."""
    
    # Test 6 SNR points for more thorough evaluation
    snr_range = [-1.0, 0.0, 1.0, 2.0, 3.0, 4.0]
    ...
    print(f'\nComparing SCL vs AI-Fast-SCL (Comprehensive Test)')
    ...
    print("="*80)
    print("✓ AI-Fast-SCL is FASTER than regular SCL!" if np.min(all_speedups) > 1.0 else "✗ Some SNRs are not faster")
    print("\nDone!")


if __name__ == '__main__':
    run_compare_comprehensive(N=128, K=64, messages=100, L=4)
